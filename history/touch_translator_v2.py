# ============================================================
# EOS P100 Touch Translator - v2 with configurable axis mapping
# iiyama T1531SR (eGalax RS232) --> iRV 5W232 protocol --> Charon
# ============================================================

import machine
import utime

BAUD = 9600

# ---- AXIS MAPPING CONFIG ----
# Toggle these to test different orientations
INVERT_X = False   # was True (255 - value). Set False to remove inversion.
INVERT_Y = False   # was True (255 - value). Set False to remove inversion.
SWAP_XY  = False   # set True if X/Y axes are swapped between the two screens
# ------------------------------

uart_in = machine.UART(0, baudrate=BAUD, tx=machine.Pin(0), rx=machine.Pin(1),
                        bits=8, parity=None, stop=1)

uart_out = machine.UART(1, baudrate=BAUD, tx=machine.Pin(4), rx=machine.Pin(5),
                         bits=8, parity=None, stop=1)

led = machine.Pin(25, machine.Pin.OUT)

print("=" * 50)
print("EOS P100 Touch Translator v2")
print("INVERT_X={} INVERT_Y={} SWAP_XY={}".format(INVERT_X, INVERT_Y, SWAP_XY))
print("=" * 50)

for _ in range(3):
    led.on()
    utime.sleep_ms(150)
    led.off()
    utime.sleep_ms(150)
led.on()

EGALAX_MAX = 16383
IRV_MAX = 255

buf = bytearray()
was_touching = False

def scale(val, invert):
    scaled = (val * IRV_MAX) // EGALAX_MAX
    if invert:
        return IRV_MAX - scaled
    return scaled

print("Listening for touch input...")
print()

while True:
    if uart_in.any():
        data = uart_in.read()
        if data:
            buf.extend(data)

            while len(buf) >= 5:
                b0, b1, b2, b3, b4 = buf[0:5]

                if not (b0 & 0x80):
                    buf = buf[1:]
                    continue

                touch = b0 & 0x01
                raw_x = (b1 & 0x7F) | ((b2 & 0x7F) << 7)
                raw_y = (b3 & 0x7F) | ((b4 & 0x7F) << 7)

                irv_x = scale(raw_x, INVERT_X)
                irv_y = scale(raw_y, INVERT_Y)

                if SWAP_XY:
                    irv_x, irv_y = irv_y, irv_x

                if touch:
                    out = bytes([0xFF, 0x00, irv_x, 0x00, irv_y])
                    uart_out.write(out)
                    print("TOUCH  iiyama X={:5d} Y={:5d}  -> iRV X={:3d} Y={:3d}".format(
                        raw_x, raw_y, irv_x, irv_y))
                    was_touching = True
                else:
                    if was_touching:
                        uart_out.write(bytes([0xFF, 0xFE, 0xFE]))
                        print("RELEASE")
                        was_touching = False

                led.toggle()
                buf = buf[5:]

    utime.sleep_ms(1)
