...
#   Mode 2  NAV: CalBar    - taps "Touchscreen calibration" bar
#                            (148,203) to reveal the start button
#   Mode 3  NAV: StartCal  - taps calibration start button *
#   Mode 4  PHYSICAL       - sample-and-hold for the 5 points (finger)
#   Mode 5  NAV: Tick      - taps the Tick/OK (80,96) as fallback
#   (wraps back to IDLE)
#
...
SERVICE1_XY   = (34, 68)
CALBAR_XY     = (148, 203)   # "Touchscreen calibration" bar - reveals start button
...
print(" 1=Service1 2=CalBar 3=StartCal 4=PHYSICAL(finger) 5=Tick")
print("=" * 50)
...
while True:
    ...
    if bootsel_pressed():
        ...
        if mode == 0:
            ...
            print("Mode 1: tapping Service 1", SERVICE1_XY)
            blink(1); send_tap(*SERVICE1_XY)
        elif mode == 2:
            print("Mode 2: tapping Touchscreen calibration bar", CALBAR_XY)
            blink(2); send_tap(*CALBAR_XY)
        elif mode == 3:
            print("Mode 3: tapping calibration START button", START_CAL_XY)
            blink(3); send_tap(*START_CAL_XY)
        elif mode == 4:
            print("Mode 4: PHYSICAL sample-and-hold - touch the 5 points")
            print("        press BOOTSEL again when done to advance")
            blink(4)
            physical_mode()   # blocks until BOOTSEL pressed again
            print("       exited physical mode")
        elif mode == 5:
            print("Mode 5: tapping Tick/OK (80,96) [fallback]")
            blink(5); send_tap(*TICK_XY)
